    							...
    							if(err){
    								...
    								client.query(sprintf("SELECT tipo_transaccion from cuenta WHERE id_cliente = '%1$s'",id_cliente), function(err, result) {
    									...
    									if(err){
    										...
    										client.query(sprintf("INSERT INTO venta (id_cliente,tipo_transaccion,dinero,volumen) VALUES('%1$s','%2$s','%3$s','%4$s')",id_cliente, transaccion,dinero_vendido,vol_vendido ),function(err,result){
    											done();
    											if(err){
    												return console.error('Error de conexion', err);
    											}else{
    												...
    												client.query("SELECT MAX(pk_id_corte) FROM corte;", function(err,result){        //consulto maximo id de venta
    														...
    														if(err){
    															...
    															client.query(sprintf("INSERT INTO venta_detalle VALUES('%1$s','%2$s','%3$s','%4$s','%5$s','%6$s','%7$s','%8$s','%9$s','%10$s','%11$s')", (last_id+1),placa,km, idproducto,precio,cara,idproducto,last_corte,(dinero_vendido).toFixed(2),vol_vendido,numvehiculo),function(err,result){
    																done();
    																if(err){
    																	return console.error('Error de conexion', err);
    																}else{
    											...
    											if(err){
    												...
    												client.query("SELECT MAX(pk_id_corte) FROM corte;", function(err,result){        //consulto maximo id de venta
    														...
    														if(err){
    															...
    															console.log("Ultimo corte: " + last_corte);
    															var precioaj =
    															client.query(sprintf("INSERT INTO venta_detalle VALUES('%1$s','%2$s','%3$s','%4$s','%5$s','%6$s','%7$s','%8$s','%9$s','%10$s')", (last_id+1),placa,km, idproducto,precio,cara,idproducto,last_corte,(dinero_vendido/100).toFixed(2),vol_vendido),function(err,result){
    																done();
    																if(err){
    																	return console.error('Error de conexion', err);
    																}else{
