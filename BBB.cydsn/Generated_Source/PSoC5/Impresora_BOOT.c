         ...
         
{
    ...
    while(bufIndex < size)
    {
        Impresora_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        *  Impresora_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(Impresora_BL_CHK_DELAY_MS);
        }
    }
    ...
    while(Impresora_GetRxBufferSize() > 0u)
    {
        ...
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            ...
            *  If not wait for additional Impresora_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != Impresora_PACKET_EOP)
            {
                CyDelay(Impresora_WAIT_EOP_DELAY);
            }
        }
