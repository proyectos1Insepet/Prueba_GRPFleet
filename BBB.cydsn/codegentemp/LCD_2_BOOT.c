         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        *  LCD_2_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(LCD_2_BL_CHK_DELAY_MS);
        }
    }
    ...
    while(LCD_2_GetRxBufferSize() > 0u)
    {
        ...
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            ...
            *  If not wait for additional LCD_2_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != LCD_2_PACKET_EOP)
            {
                CyDelay(LCD_2_WAIT_EOP_DELAY);
            }
        }
