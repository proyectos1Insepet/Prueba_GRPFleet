         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        *  LCD_1_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(LCD_1_BL_CHK_DELAY_MS);
        }
    }
    ...
    while(LCD_1_GetRxBufferSize() > 0u)
    {
        ...
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            ...
            *  If not wait for additional LCD_1_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != LCD_1_PACKET_EOP)
            {
                CyDelay(LCD_1_WAIT_EOP_DELAY);
            }
        }
